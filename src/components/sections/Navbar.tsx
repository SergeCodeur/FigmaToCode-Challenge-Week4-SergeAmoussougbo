...
const Navlinks = ({ handleLinkClick, toggleNav }: Props) => {
  return (
    <ul
      className={`inline-flex gap-8 max-md:fixed max-md:top-0 max-md:left-0 max-md:h-screen max-md:w-full max-md:flex-col max-md:justify-center max-md:items-center max-md:bg-white max-md:z-50 max-md:-translate-x-full transition-transform ' ${
    ...
    >
      {Routes.map((route) => (
        <li key={route.path}>
          <Link
            className="text-heading-h5-600 cursor-pointer"
            href={route.path}
  ...
  return (
    <MaxContentWidthWrapper className="py-0">
      <nav className="w-full flex justify-between items-center h-14 lg:px-8 relative">
        ...
        <Button className="hidden lg:flex" variant="medium">
          <span>Resume</span>
          <svg
            width="20"
            height="20"
